...
# I won't need a Heisenberg compensator, but I might need a temporal incursion compensator.
my @locations;
...
for(my $incursiony=1; $incursiony < $#contraption; $incursiony++) {
  for(my $incursionx=1; $incursionx < $#{$contraption[$incursiony]}; $incursionx++) {
    next unless $contraption[$incursiony][$incursionx] eq '.';
    $contraption[$incursiony][$incursionx] = '#';
    ...
    while($#locations >= 0) {
      if($DEBUG) {
        for(my $y=1; $y < $#contraption; $y++) {
          for(my $x=1; $x < $#{$contraption[$y]}; $x++) {
            ...
            } elsif($heatmap[$y][$x][0] > 0 || $heatmap[$y][$x][1] > 0 || $heatmap[$y][$x][2] > 0 || $heatmap[$y][$x][3] > 0) {
              print 'X';
            } else {
      ...
      $heatmap[$y][$x][$direction] = 1 unless $contraption[$y][$x] ne '.';
