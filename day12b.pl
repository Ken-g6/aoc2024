...
for(my $y=0; $y < $#matrix; $y++) {
  ...
  for(my $x=0; $x < $xsize; $x++) {
    ++$area{$matrix[$y][$x]};
    if($matrix[$y][$x] != $matrix[$y+1][$x] && ($matrix[$y][$x] != $lasty0 || $matrix[$y+1][$x] != $lasty1)) {
      $perimiter{$matrix[$y][$x]}++ unless($matrix[$y][$x] == $lasty0 && $matrix[$y][$x] != $lasty1);
      $perimiter{$matrix[$y+1][$x]}++ unless($matrix[$y+1][$x] == $lasty1 && $matrix[$y+1][$x] != $lasty0);
    }
    $lasty0 = $matrix[$y][$x];
    $lasty1 = $matrix[$y+1][$x];
  }
...
for(my $x=0; $x < $xsize; $x++) {
  ...
  my $lastx1 = -1;
  for(my $y=0; $y < $#matrix; $y++) {
    if($matrix[$y][$x] != $matrix[$y][$x+1] && ($matrix[$y][$x] != $lastx0 || $matrix[$y][$x+1] != $lastx1)) {
      $perimiter{$matrix[$y][$x]}++ unless($matrix[$y][$x] == $lastx0 && $matrix[$y][$x] != $lastx1);
      $perimiter{$matrix[$y][$x+1]}++ unless($matrix[$y][$x+1] == $lastx1 && $matrix[$y][$x+1] != $lastx0);
    }
    $lastx0 = $matrix[$y][$x];
    $lastx1 = $matrix[$y][$x+1];
  }
